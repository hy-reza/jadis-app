...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Uspkelahiran.search')}}" method="get">
                <input type="text" name="Uspkelahiran" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Uspkelahiran.create')}}" class="btn btn-primary btn-sm">Tambah Data Pernyataan Kelahiran</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Uspkelahiran)
										<tr>
											<td>{{$Uspkelahiran['id']}}</td>
											<td>{{$Uspkelahiran['nama']}}</td>
											<td>{{$Uspkelahiran['nik']}}</td>
                      <td>{{$Uspkelahiran['nomor_kk']}}</td>
                      <td>{{$Uspkelahiran['jenis_kelamin']}}</td>
                      <td>{{$Uspkelahiran['pekerjaan']}}</td>
                      <td>{{$Uspkelahiran['alamat']}}</td>
                      <td>{{$Uspkelahiran['nama_anak']}}</td>
                      <td>{{$Uspkelahiran['ttl']}}</td>
                      <td>{{$Uspkelahiran['kelahiran_ke']}}</td>
                      <td>{{$Uspkelahiran['nama_ayah']}}</td>
                      <td>{{$Uspkelahiran['nama_ibu']}}</td>
                      <td>{{$Uspkelahiran['pekerjaan_ortu']}}</td>
                      <td>{{$Uspkelahiran['alamat_ortu']}}</td>
                      <td>{{$Uspkelahiran['tgl_hari_ini']}}</td>
                      <td>{{$Uspkelahiran['nama_kades']}}</td>
                      <td>{{$Uspkelahiran['nama_pernyataan']}}</td>
											<td><a href="{{action('UspkelahiranController@edit', $Uspkelahiran['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('UspkelahiranController@show', $Uspkelahiran['nik'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('UspkelahiranController@destroy', $Uspkelahiran['id'])}}" method="post">
													@csrf
