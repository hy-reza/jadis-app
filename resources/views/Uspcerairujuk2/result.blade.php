...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Uspcerairujuk2.search')}}" method="get">
                <input type="text" name="Uspcerairujuk2" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Uspcerairujuk2.create')}}" class="btn btn-primary btn-sm">Tambah Data Pengajuan Cerai Rujuk</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Uspcerairujuk2)
										<tr>
											<td>{{$Uspcerairujuk2['id']}}</td>
                      <td>{{$Uspcerairujuk2['nomer_surat']}}</td>
											<td>{{$Uspcerairujuk2['nama']}}</td>
											<td>{{$Uspcerairujuk2['nik']}}</td>
                      <td>{{$Uspcerairujuk2['ttl']}}</td>
                      <td>{{$Uspcerairujuk2['pekerjaan']}}</td>
                      <td>{{$Uspcerairujuk2['alamat']}}</td>
                      <td>{{$Uspcerairujuk2['nama_sah']}}</td>
                      <td>{{$Uspcerairujuk2['nik_sah']}}</td>
                      <td>{{$Uspcerairujuk2['ttl_sah']}}</td>
                      <td>{{$Uspcerairujuk2['pekerjaan_sah']}}</td>
                      <td>{{$Uspcerairujuk2['alamat_sah']}}</td>
                      <td>{{$Uspcerairujuk2['suami']}}</td>
                      <td>{{$Uspcerairujuk2['negara']}}</td>
                      <td>{{$Uspcerairujuk2['tgl_hari_ini']}}</td>
                      <td>{{$Uspcerairujuk2['nama_kades']}}</td>
											<td><a href="{{action('Uspcerairujuk2Controller@edit', $Uspcerairujuk2['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('Uspcerairujuk2Controller@show', $Uspcerairujuk2['id'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('Uspcerairujuk2Controller@destroy', $Uspcerairujuk2['id'])}}" method="post">
													@csrf
