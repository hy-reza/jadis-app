...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Uspkematian.search')}}" method="get">
                <input type="text" name="Uspkematian" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Uspkematian.create')}}" class="btn btn-primary btn-sm">Tambah Data Pernyataan Kematian</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Uspkematian)
										<tr>
											<td>{{$Uspkematian['id']}}</td>
											<td>{{$Uspkematian['nama']}}</td>
											<td>{{$Uspkematian['nik']}}</td>
                      <td>{{$Uspkematian['nomor_kk']}}</td>
                      <td>{{$Uspkematian['jenis_kelamin']}}</td>
                      <td>{{$Uspkematian['pekerjaan']}}</td>
                      <td>{{$Uspkematian['alamat']}}</td>
                      <td>{{$Uspkematian['status_hubungan_keluarga']}}</td>
                      <td>{{$Uspkematian['nama_meninggal']}}</td>
                      <td>{{$Uspkematian['nik_meninggal']}}</td>
                      <td>{{$Uspkematian['nomor_kk_meninggal']}}</td>
                      <td>{{$Uspkematian['ttl_meninggal']}}</td>
                      <td>{{$Uspkematian['alamat_meninggal']}}</td>
                      <td>{{$Uspkematian['tanggal_meninggal']}}</td>
                      <td>{{$Uspkematian['pukul_meninggal']}}</td>
                      <td>{{$Uspkematian['dialamat_meninggal']}}</td>
                      <td>{{$Uspkematian['tgl_hari_ini']}}</td>
                      <td>{{$Uspkematian['nama_pernyataan']}}</td>
                      <td>{{$Uspkematian['nama_rt']}}</td>
                      <td>{{$Uspkematian['nama_kades']}}</td>
                      <td>{{$Uspkematian['nama_rw']}}</td>
											<td><a href="{{action('UspkematianController@edit', $Uspkematian['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('UspkematianController@show', $Uspkematian['nik'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('UspkematianController@destroy', $Uspkematian['id'])}}" method="post">
													@csrf
