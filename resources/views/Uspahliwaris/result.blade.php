...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Uspahliwaris.search')}}" method="get">
                <input type="text" name="Uspahliwaris" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Uspahliwaris.create')}}" class="btn btn-primary btn-sm">Tambah Data Pernyataan Ahli Waris</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Uspahliwaris)
										<tr>
											<td>{{$Uspahliwaris['id']}}</td>
                      <td>{{$Uspahliwaris['nomer_surat']}}</td>
											<td>{{$Uspahliwaris['nama_alm']}}</td>
											<td>{{$Uspahliwaris['nomer_akte_kematian']}}</td>
                      <td>{{$Uspahliwaris['hari']}}</td>
                      <td>{{$Uspahliwaris['tanggal']}}</td>
                      <td>{{$Uspahliwaris['bulan']}}</td>
                      <td>{{$Uspahliwaris['tahun']}}</td>
                      <td>{{$Uspahliwaris['status_suami_istri']}}</td>
                      <td>{{$Uspahliwaris['nama_suami_istri']}}</td>
                      <td>{{$Uspahliwaris['nama_anak']}}</td>
                      <td>{{$Uspahliwaris['umur_anak']}}</td>
                      <td>{{$Uspahliwaris['pekerjaan_anak']}}</td>
                      <td>{{$Uspahliwaris['alamat_anak']}}</td>
                      <td>{{$Uspahliwaris['no_req']}}</td>
                      <td>{{$Uspahliwaris['tgl_hari_ini']}}</td>
                      <td>{{$Uspahliwaris['nama_camat']}}</td>
                      <td>{{$Uspahliwaris['nama_kades']}}</td>
											<td><a href="{{action('UspahliwarisController@edit', $Uspahliwaris['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('UspahliwarisController@show', $Uspahliwaris['id'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('UspahliwarisController@destroy', $Uspahliwaris['id'])}}" method="post">
													@csrf
