...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Uspcerairujuk1.search')}}" method="get">
                <input type="text" name="Uspcerairujuk1" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Uspcerairujuk1.create')}}" class="btn btn-primary btn-sm">Tambah Data Pengajuan Cerai Rujuk</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Uspcerairujuk1)
										<tr>
											<td>{{$Uspcerairujuk1['id']}}</td>
                      <td>{{$Uspcerairujuk1['nomer_surat']}}</td>
											<td>{{$Uspcerairujuk1['nama']}}</td>
											<td>{{$Uspcerairujuk1['umur']}}</td>
                      <td>{{$Uspcerairujuk1['pekerjaan']}}</td>
                      <td>{{$Uspcerairujuk1['alamat']}}</td>
                      <td>{{$Uspcerairujuk1['nama_sah']}}</td>
                      <td>{{$Uspcerairujuk1['umur_sah']}}</td>
                      <td>{{$Uspcerairujuk1['pekerjaan_sah']}}</td>
                      <td>{{$Uspcerairujuk1['alamat_sah']}}</td>
                      <td>{{$Uspcerairujuk1['lama_tahun']}}</td>
                      <td>{{$Uspcerairujuk1['lama_bulan']}}</td>
                      <td>{{$Uspcerairujuk1['tgl_hari_ini']}}</td>
                      <td>{{$Uspcerairujuk1['nama_pelapor']}}</td>
                      <td>{{$Uspcerairujuk1['nama_kades']}}</td>
											<td><a href="{{action('Uspcerairujuk1Controller@edit', $Uspcerairujuk1['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('Uspcerairujuk1Controller@show', $Uspcerairujuk1['id'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('Uspcerairujuk1Controller@destroy', $Uspcerairujuk1['id'])}}" method="post">
													@csrf
