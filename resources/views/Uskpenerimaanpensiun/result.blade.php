...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Uskpenerimaanpensiun.search')}}" method="get">
                <input type="text" name="Uskpenerimaanpensiun" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Uskpenerimaanpensiun.create')}}" class="btn btn-primary btn-sm">Tambah Data Ket Penerimaan Pensiun</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Uskpenerimaanpensiun)
										<tr>
											<td>{{$Uskpenerimaanpensiun['id']}}</td>
                      <td>{{$Uskpenerimaanpensiun['nomer_surat']}}</td>
											<td>{{$Uskpenerimaanpensiun['nama_lengkap']}}</td>
											<td>{{$Uskpenerimaanpensiun['jenis_kelamin']}}</td>
                      <td>{{$Uskpenerimaanpensiun['ttl']}}</td>
                      <td>{{$Uskpenerimaanpensiun['kewarganegaraan']}}</td>
                      <td>{{$Uskpenerimaanpensiun['agama']}}</td>
                      <td>{{$Uskpenerimaanpensiun['status_perkawinan']}}</td>
                      <td>{{$Uskpenerimaanpensiun['pekerjaan']}}</td>
                      <td>{{$Uskpenerimaanpensiun['alamat']}}</td>
                      <td>{{$Uskpenerimaanpensiun['dipergunakaan_untuk']}}</td>
                      <td>{{$Uskpenerimaanpensiun['tujuan']}}</td>
                      <td>{{$Uskpenerimaanpensiun['surat_pengantar']}}</td>
                      <td>{{$Uskpenerimaanpensiun['daf_kk']}}</td>
                      <td>{{$Uskpenerimaanpensiun['no_ktp']}}</td>
                      <td>{{$Uskpenerimaanpensiun['nama_bin']}}</td>
                      <td>{{$Uskpenerimaanpensiun['tgl_hari_ini']}}</td>
                      <td>{{$Uskpenerimaanpensiun['nama_pemegang']}}</td>
                      <td>{{$Uskpenerimaanpensiun['nama_kades']}}</td>

											<td><a href="{{action('UskpenerimaanpensiunController@edit', $Uskpenerimaanpensiun['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('UskpenerimaanpensiunController@show', $Uskpenerimaanpensiun['id'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('UskpenerimaanpensiunController@destroy', $Uskpenerimaanpensiun['id'])}}" method="post">
													@csrf
