...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container">
              <form action="{{ route('Usktidakmampu1.search')}}" method="get">
                <input type="text" name="Usktidakmampu1" class="form-control" placeholder="Search..." style="width: 80%; display: inline;">
            ...
            <div class="row m-t-25">
                <div class="col-sm-5 col-lg-3">
                    <table style="width: 100px" id="datatable" class="table table-striped table-bordered">
                            <tr>
                                <td>
                                    <a href="{{route ('Usktidakmampu1.create')}}" class="btn btn-primary btn-sm">Tambah Data Ket Tidak Mampu</a>
                                </td>
                            </tr>
                    </table>
                    </div>
                      ...
                      <th>AKSI</th>
									...
									<tbody>
                    @foreach($result as $Usktidakmampu1)
										<tr>
											<td>{{$Usktidakmampu1['id']}}</td>
                      <td>{{$Usktidakmampu1['nomer_surat']}}</td>
											<td>{{$Usktidakmampu1['nama']}}</td>
											<td>{{$Usktidakmampu1['jenis_kelamin']}}</td>
                      <td>{{$Usktidakmampu1['ttl']}}</td>
                      <td>{{$Usktidakmampu1['kewarganegaraan']}}</td>
                      <td>{{$Usktidakmampu1['agama']}}</td>
                      <td>{{$Usktidakmampu1['status']}}</td>
                      <td>{{$Usktidakmampu1['pekerjaan']}}</td>
                      <td>{{$Usktidakmampu1['alamat']}}</td>
                      <td>{{$Usktidakmampu1['persyaratan']}}</td>
                      <td>{{$Usktidakmampu1['tgl_hari_ini']}}</td>
                      <td>{{$Usktidakmampu1['nama_berkepentingan']}}</td>
                      <td>{{$Usktidakmampu1['nama_kades']}}</td>
                      <td>{{$Usktidakmampu1['no']}}</td>
                      <td>{{$Usktidakmampu1['nama_camat']}}</td>
											<td><a href="{{action('Usktidakmampu1Controller@edit', $Usktidakmampu1['id'])}}" class="btn btn-warning">Ubah</a><br><br>
                        <a href="{{action('Usktidakmampu1Controller@show', $Usktidakmampu1['id'])}}" class="btn btn-warning">Surat</a><br><br>
												<form action="{{action('Usktidakmampu1Controller@destroy', $Usktidakmampu1['id'])}}" method="post">
													@csrf
